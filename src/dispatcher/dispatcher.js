...
const dispatcher = _dispatcher.asObservable().shareReplay(1)
...
function buildFilterFunction(args) {
  ...
  let notFoundKey = ''
  const hasActions = Object.keys(args).some(
    key => Object.keys(Actions).indexOf((notFoundKey = args[key])) !== -1
  )

  if (!hasActions) {
    throw new Error(
      `Invalid filters provided to dispatcher func: '${notFoundKey}'`
    )
  }

  return message => {
    // If filter args have actions to filter by them
    return Object.keys(args).some(key => args[key] === message.type)
  }
}
...
export function getAction(...args) {
  ...
  if (args.length === 0) {
    ...
    filteredDispatcher = dispatcher.filter(args[0])
  } else {
    // Sugaring for filtering by actions
    // arguments's values are the actions we would like to filter by
    filteredDispatcher = dispatcher.filter(buildFilterFunction(args))
  }
...
export default function getPayload(...args) {
  ...
  // default
  return getAction(...args).do((actionObject) => {
    return actionObject
  })
    .pluck('payload')
}
